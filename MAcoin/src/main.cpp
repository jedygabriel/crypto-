...
#if defined(NDEBUG)
# error "MAcoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MAcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MAcoin Signed Message:\n";
...
{
    // MAcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // MAcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4.20 * 24 * 60 * 60; // MAcoin: 4.20 days
static const int64 nTargetSpacing = 2.666 * 60; // MAcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MAcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MAcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MAcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        //     CTxOut(nValue=50.00000000, scriptPubKey=212344710fa689ad5023690c80f3a4)
        //   vMerkleTree: 97ddfbbae6

        // Genesis block
        const char* pszTimestamp = "-> MAcoin <-";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xe9084ee9cc0646f109c8fe134482a7ed2a174d3e71ec36656e2eddd076f41d10"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // MAcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MAcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MAcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MAcoinMiner(CWallet *pwallet)
{
    printf("MAcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running MAcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MAcoinMiner, pwallet));
}
